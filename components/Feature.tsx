...
function Feature() {
    return (
        <section className={"bg-feature-bg bg-center bg-no-repeat py-24 flex-col flexCenter overflow-hidden "}>
                ...
                <div className="z-20 flex w-full flex-col lg:w-[60%]">
                    ...
                    <ul className="mt-10 grid gap-10 md:grid-cols-2 lg:mt-20 lg:gap-20">
                        {FEATURES.map((feature) => (
                            <FeatureItem title={feature.title} icon={feature.icon} description={feature.description} key={feature.title} variant={feature.variant}/>
                        ))}
                    </ul>
                </div>
            </div>
...
function FeatureItem({ title, icon, description, variant }: FeatureItemProps) {
    return (
